...
python3 ~/.claude/skills/stock_screener/screener.py --setup all --timeframe weekly  # a different horizon, not a smoothed daily
...
### `--timeframe weekly` is a different horizon, not a smoothed daily

```bash
python3 stock_screener/screener.py --setup all                     # daily (default)
python3 stock_screener/screener.py --setup all --timeframe weekly  # weekly bars
```

The flag does not smooth the daily chart or filter noise out of a daily signal, and a
weekly COILED is not "a daily COILED, confirmed". It asks the same five predicates of a
chart whose every bar is a week — a different question about a different span of market
history — and the answer is usually a different set of names rather than a subset.

- **The bars.** Daily reads 2 years of daily bars. Weekly reads **10 years** of weekly
  bars, not 5: five years of weekly data is 262 bars, and a 200-period average would eat
  199 of them and leave 63 usable — fewer than the 90-bar thrust window. Ten years gives
  523 bars and 324 usable, the same working depth the daily path has.
- **Periods stay bar counts.** 20/50/100/200 are periods on whatever the primary timeframe
  ...
  about a year. That is the intended meaning, not an oversight. The 50-bar up/down volume
  ratio becomes a year of weeks and the 250-bar 52-period range about five years.
- **Momentum is weekly-only.** A daily scan blends the daily RSI and MACD with the weekly
  ones confirming them. Weekly has nothing above it, so the confirmation terms are absent
  — deliberately *not* filled in by passing the weekly series twice, which would count one
  reading as two agreeing timeframes and award 2.5 points where a daily name earns 1.0 for
  the same fact. The honest consequence is that weekly momentum spans 2.5–7.5 rather than
  0–10 and pulls the weighted total toward neutral: half the evidence really is missing.
- **Relative strength is measured in weeks.** The two RS columns are 21 and 63 *primary
  bars* against the Nifty 50 over the same window, and the benchmark is fetched on the
  scan's own interval so both sides of the subtraction agree. On weekly they are 21 and 63
  weeks, though the column headers still read 1-month and 3-month.
- **A newly listed name has no weekly history.** The engine needs 60 primary bars — 60
  weeks — so recent listings land in the FAILED list on a weekly scan and nowhere on a
  daily one. 21 of the Nifty 500 did on 2026-08-03.

Everything else — thrust detection, the consolidation anchor, the fractals, the rejection
zones, the volume profile, the 52-period range and the risk-reward gate with its 1.5×ATR
stop — already reads the primary series, so it follows the timeframe without changing
meaning.

Counts move a lot, and not in one direction. On the live Nifty 500, 2026-08-03, loosened:
...
The intuition that a weekly reversal bar must be rarer than a daily one is wrong, and
PULLBACK is the setup that shows it: a week closing in the top half of its own range is no
less common than a day doing so, and weekly swings clear the 3% retracement gate more
easily. What genuinely collapses is anything needing a recent dated *event* — a breakout
bar, a fresh 50/200 cross — because the same calendar window holds a fifth as many bars.
CONFLUENCE collapses hardest, needing two of those to land on one name in one week.

The timeframe is recorded in three places, so a weekly scan and a daily scan can never be
read as one file: the scan header's first line, the `timeframe` field of `--json`, and the
...
`--append` writes the header only when the file is new or empty; the `threshold_mode`
column records `strict` or `loosened` and `bar_timeframe_daily_or_weekly` records which
bars produced the row, so two differently-thresholded scans — or a daily and a weekly one —
cannot be read as one. The timeframe column matters more than the mode one: loosened and
strict measure the same thing at two sensitivities, while daily and weekly measure two
different horizons and every number in the row changes meaning with it.
...
documentation beside them. So every one of the 32 headers carries its own meaning and
...
Each carries the same 32 columns and the same 20-row cap, holding only that setup's rows.
